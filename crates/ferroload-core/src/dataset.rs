...
//! ```text
//! <root>/manifest.json
//! <root>/index/index.json            (scaffold; parquet in production)
//! <root>/shards/shard-00000.tar (+ .tar.idx)
//! <root>/versions/vN.json
//! ```

use crate::error::{Error, Result};
use crate::index::{IndexBackend, IndexReader, IndexRow, JsonIndex};
use crate::manifest::{Manifest, Modality};
...
impl DatasetWriter {
    ...
    /// Finalize: write index, manifest (atomically, last), and a version snapshot.
    pub fn close(mut self) -> Result<Manifest> {
        self.finish_shard()?;

        let index_rel = "index/index.json";
        JsonIndex.write(&self.root.join(index_rel), &self.rows)?;

        let n_shards = (self.shard_id + 1).max(0) as u32;
        self.manifest.index.path = index_rel.to_string();
        self.manifest.index.rows = self.rows.len() as u64;
...
/// Reader over a self-contained dataset root (base + any enrichment layers).
pub struct Dataset {
    root: PathBuf,
    manifest: Manifest,
    index: IndexReader,
    ...
    /// Open shard handles keyed by absolute path (Arc -> lock-free positional reads).
    shards: std::sync::Mutex<std::collections::HashMap<PathBuf, std::sync::Arc<std::fs::File>>>,
}
...
impl Dataset {
    pub fn open(root: impl AsRef<Path>) -> Result<Self> {
        ...
        for (li, lref) in manifest.layers.iter().enumerate() {
            ...
            layers.push(LoadedLayer {
                shards_dir: root.join(&lref.shards_dir),
                rows: lrows,
                by_pos,
        ...
        Ok(Dataset {
            root,
            manifest,
            index: IndexReader::new(rows),
            layers,
            modality_layer,
            shards: std::sync::Mutex::new(std::collections::HashMap::new()),
        })
    }

    /// Get (or lazily open) a cached shard handle by absolute path.
    fn shard_file(&self, path: &Path) -> Result<std::sync::Arc<std::fs::File>> {
        let mut guard = self.shards.lock().unwrap();
        if let Some(f) = guard.get(path) {
            return Ok(f.clone());
        }
        let f = std::sync::Arc::new(std::fs::File::open(path)?);
        guard.insert(path.to_path_buf(), f.clone());
        Ok(f)
    }

    /// Positional read of a member at `path` (cached handle, `read_exact_at`).
    fn read_member_path(&self, path: &Path, off: u64, len: u64) -> Result<Vec<u8>> {
        let f = self.shard_file(path)?;
    ...
    fn base_shard_path(&self, shard_id: u32) -> PathBuf {
        self.root.join("shards").join(format!("shard-{:05}.tar", shard_id))
    }
    /// Shard path for a layer row: an explicit `shard` filename when present
    /// (partitioned layers), else the canonical `shard-{id:05}.tar`.
    fn layer_shard_path(dir: &Path, row: &IndexRow) -> PathBuf {
        match &row.shard {
            Some(name) => dir.join(name),
            None => dir.join(format!("shard-{:05}.tar", row.shard_id)),
        }
    }

    /// Resolve `(shard_path, off, len)` for sample position `i` + `modality`,
    /// across the base dataset and enrichment layers. `None` if absent.
    fn resolve(&self, i: usize, modality: &str) -> Result<Option<(PathBuf, u64, u64)>> {
        let (sample_id, base_hit) = {
            let row = self.index.get(i)?;
            (row.sample_id, row.offsets.get(modality).copied())
        };
        if let Some([o, l]) = base_hit {
            return Ok(Some((self.base_shard_path(self.index.get(i)?.shard_id), o, l)));
        }
        if let Some(&li) = self.modality_layer.get(modality) {
            ...
            if let Some(lrow) = layer.row_for(sample_id) {
                if let Some([o, l]) = lrow.offsets.get(modality) {
                    return Ok(Some((Self::layer_shard_path(&layer.shards_dir, lrow), *o, *l)));
                }
            }
        }
        Ok(None)
    ...
    pub fn read_blob(&self, i: usize, modality: &str) -> Result<Option<Vec<u8>>> {
        match self.resolve(i, modality)? {
            Some((path, off, len)) => Ok(Some(self.read_member_path(&path, off, len)?)),
            None => Ok(None),
        }
    }

    /// Read one modality for many samples into a **single contiguous buffer**,
    /// returning the buffer plus per-sample `(offset, length)` spans. One
    /// allocation + one copy total (vs N small buffers); missing modalities get a
    /// zero-length span. The batched read path for a DataLoader worker.
    pub fn read_blobs_contig(
        &self,
        indices: &[usize],
        modality: &str,
    ) -> Result<(Vec<u8>, Vec<(usize, usize)>)> {
        // first pass: resolve locations + compute layout
        let mut plan: Vec<Option<(PathBuf, u64, u64)>> = Vec::with_capacity(indices.len());
        ...
        // second pass: read each member directly into its slice
        let mut buf = vec![0u8; total];
        for (k, item) in plan.iter().enumerate() {
            if let Some((path, off, _len)) = item {
                let (start, l) = spans[k];
                let f = self.shard_file(path)?;
        ...
        for m in &selected {
            match self.resolve(i, m)? {
                Some((path, off, len)) => {
                    ...
                    blobs.insert(m.clone(), self.read_member_path(&path, off, len)?);
                }
                None => {
...
impl Dataset {
    /// Read just the index row (no shard I/O) — fast metadata access.
    pub fn row(&self, i: usize) -> Result<&IndexRow> {
        self.index.get(i)
    }

    /// Subset by a `WHERE` predicate over inline metadata; returns matching
    /// `sample_id`s in ascending order (deterministic, see DESIGN §6).
    pub fn subset(&self, where_sql: &str) -> Result<Vec<u64>> {
        crate::subset::subset_ids(self.index.rows(), where_sql)
    }
    ...
    pub fn verify(&self) -> Result<usize> {
        ...
        for i in 0..self.len() {
            for m in &mods {
                if let Some((path, off, len)) = self.resolve(i, m)? {
                    let bytes = self.read_member_path(&path, off, len)?;
...
fn layer_part_index_rel(name: &str, part: u32) -> String {
    format!("index/{name}.part-{part}.json")
}
...
///   layer index `index/<name>.json`, and registers the layer in the manifest on
...
///   `index/<name>.part-<part>.json` and a `.done` marker on `close()`, touching
///   **no** shared state. A final [`LayerWriter::commit`] merges all fragments
///   into `index/<name>.json` and registers the layer (the one sync point).
...
impl LayerWriter {
    ...
    fn open_internal(
        ...
        let (rows, shard_id) = if index_abs.exists() {
            let existing = JsonIndex.read(&index_abs)?;
    ...
    /// Finalize this writer. Single-process: write `index/<name>.json` and register
    ...
    pub fn close(mut self) -> Result<()> {
        self.finish_shard()?;
        JsonIndex.write(&self.root.join(&self.index_rel), &self.rows)?;
        match self.partition {
            Some(p) => {
    ...
    /// `index/<name>.part-*.json` fragment (and any prior `index/<name>.json`)
    ...
    pub fn commit(
        ...
        if merged_abs.exists() {
            for r in JsonIndex.read(&merged_abs)? {
                merged.insert(r.sample_id, r);
            }
        }

        // collect fragment files: index/<name>.part-*.json
        ...
        JsonIndex.write(&merged_abs, &rows)?;
